...
export const merchantTxnAPI = {
  ...
  // ── Import ──
  importCsv:(formData)     => client.post('/merchant-transactions/import?preview=true', formData, {
    headers: { 'Content-Type': 'multipart/form-data' }
  }),
  importConfirm: (items)   => client.post('/merchant-transactions/import-confirm', { items }),
  ...
  compute: (d) => {
    ...
    const labourAmount      = r2(labourHeadCount * labourCharge);
    const netPayable        = r2(grossAmount - labourAmount);
    const finalPayable      = r2(netPayable - advancePayment);
    const balance           = finalPayable;

    return { lessQty, netQty, grossAmount, labourAmount, netPayable, finalPayable, balance };
  },
